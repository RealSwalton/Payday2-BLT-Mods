...
function BlackMarketGui:mouse_pressed(button, x, y)
	...
	if slot_data and self._enabled and not data.is_loadout and not self._renaming_item and self._highlighted and button == ids_zero then
		...
		if inside == true then
			if (ctg == 'masks' and data.topic_id ~= 'bm_menu_buy_mask_title') or ((ctg == 'primaries' or ctg == 'secondaries') and not data.buying_weapon and data.topic_id ~= 'bm_menu_blackmarket_title') then
