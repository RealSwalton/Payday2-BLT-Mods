...
function EnhancedHitmarkers:IsTextureOverridden(texture)
	local result = false
...
function EnhancedHitmarkers:CopyTexture(src_name, dst_name)
	local result = false
...
function EnhancedHitmarkers:DialogRestartRequired()
...
Hooks:Add('MenuManagerOnOpenMenu', 'MenuManagerOnOpenMenu_EnhancedHitmarkers', function(menu_manager, menu, position)
	if menu == 'menu_main' then
		if EnhancedHitmarkers._hit_texture_overrides_not_initialized or EnhancedHitmarkers._kill_texture_overrides_not_initialized then
			local notif = {
				title = managers.localization:text('eh_options_menu_title'),
				text = managers.localization:text('eh_options_info_popup_message_restart_required'),
				priority = 1000
			}
			BLT.Notifications:add_notification(notif)
		end
	end
end)
...
Hooks:Add('MenuManagerInitialize', 'MenuManagerInitialize_EnhancedHitmarkers', function(menu_manager)
	...
	MenuCallbackHandler.EnhancedHitmarkersChangedFocus = function(node, focus)
		if focus then
			...
			if EnhancedHitmarkers._hit_texture_overrides_not_initialized or EnhancedHitmarkers._kill_texture_overrides_not_initialized then
				EnhancedHitmarkers:DialogRestartRequired()
				if not managers.hud then
					setup:quit()
				end
			else
	...
	MenuCallbackHandler.EnhancedHitmarkersSetTextureHit = function(this, item)
		...
		if EnhancedHitmarkers:CopyTexture(EnhancedHitmarkers.override_path..texture_name, EnhancedHitmarkers.override_path .. 'hitconfirm.texture') then
	...
	MenuCallbackHandler.EnhancedHitmarkersSetTextureKill = function(this, item)
		local texture_name = EnhancedHitmarkers.texture_list[item:value()]
		if EnhancedHitmarkers:CopyTexture(EnhancedHitmarkers.override_path..texture_name, EnhancedHitmarkers.override_path .. 'hitconfirm_crit.texture') then
